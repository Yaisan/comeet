#!/bin/bash
if [ -z "$*" ]; then echo "A domain has not been set"; else

  if [ "$EUID" -ne 0 ];
    if ! dpkg -s docker.io &> /dev/null;then sudo apt install -y docker.io 
    fi
  ...
  then
    ...
    if ! dpkg -s docker.io &> /dev/null;then apt install -y docker.io
    fi  
